...
- ✅ **Integración implementada en el OS del negocio** (rama `feature/leads-a-migracion-rpc`): funnel portado a `metacrypto-os-app/apps/inbox` como ruta pública `/quiz`, persistencia real en Supabase local (migraciones `0068`/`0069`/`0070` + reconciliación `0071` con RPC transaccional `registrar_diagnostico`), módulo privado `/leads` con permiso propio, vinculación post-venta con validación de teléfono + rollback auditado, y descarte del lead sin venta
- ✅ Fases 0–C cerradas y Fase D cierre local (ver `docs/12` §6) — pendiente solo externo (Miled + negocio)
- ✅ Suite del OS: 1.329 tests en verde · `tsc` y `eslint` limpios
...
- ⏳ Pendiente externo: hosting/rate limit definitivo, confirmación de migraciones con Miled, upgrade de Next 16 (queda una alta de postcss), videos reales, alerta de triaje, PDF por email (Resend). Correcciones de seguridad preexistentes del OS listadas en `docs/13` §4
