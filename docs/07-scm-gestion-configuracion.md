...
- **Documentación (`docs/`):** `NN-nombre-kebab-case.md` — prefijo numérico de dos dígitos marcando el orden de lectura. Un doc nuevo usa el siguiente número libre (los saltos corresponden a docs jubilados, ver abajo).
  - **Docs jubilados:** a `docs-obsoletos/` (raíz del repo) con su entrada en el README de esa carpeta explicando por qué y dónde vive su contenido vigente; **no renombrar el archivo vivo**, porque los índices y referencias apuntan al mismo nombre. Las fuentes primarias crudas viven en `docs-obsoletos/fuentes-primarias/`.
...
- Las fuentes primarias crudas de la raíz (`Transcripcion_Reunion.md`, `transcripcion_audio.md`) ya fueron **jubiladas** a `docs-obsoletos/fuentes-primarias/` (síntesis vigente en `docs/00` y `docs/11`); el contenido nuevo entra directo a `docs/`.
...
```
docs/                # documentación viva (spec-first: cambiar aquí, luego codificar; índice: docs/README.md)
docs-obsoletos/      # documentación jubilada + fuentes primarias (trazabilidad; no usar como spec)
metacrypto-os-app/   # sistema interno del negocio (copia) — acá vive la implementación vigente (/quiz + /leads)
prototipos/          # material de referencia: NO es código, no se limpia ni refactoriza
scripts/             # utilidades (transcribe.py, etc.)
```

- Nunca borrar material de `prototipos/referencia/`, las transcripciones ni los docs jubilados: son fuente primaria/trazabilidad.
...
Acordado con Miled — la propuesta quedó en la reunión de integración (transcripción en `docs-obsoletos/fuentes-primarias/`), sintetizada hoy en `docs/08`:
...
- **Spec-first:** todo cambio de requisitos entra primero como commit de `docs/`; la implementación lo referencia (ej. `feat: ... (docs/11 §5)`).
