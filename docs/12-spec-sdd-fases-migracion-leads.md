...
| A — Migración + RPCs | ✅ `0068_quiz_leads.sql` (3 tablas + RLS + definición `diagnostico-cripto-v1-a` + RPC `registrar_diagnostico`) y `0069_vinculacion_validacion_rollback.sql` (validación de teléfono + auditoría + `desvincular_lead`). 13 tests de integración | — |
...
| D — Pre-producción | ☐ pendiente: checklist `docs/09`, rate limit definitivo según hosting, PR + entrega a Miled (aplicar 0068 y 0069 ANTES del deploy), smoke test | — |
