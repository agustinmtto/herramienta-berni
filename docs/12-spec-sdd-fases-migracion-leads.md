...
| C — Módulo /leads | 🟡 Criterios locales completos: permiso `leads` propio + grupo "Captación" en el nav, listado server-side con filtros GET (estado, calificación, banda de capital, fechas, UTMs, paso de abandono, búsqueda, paginación), detalle con snapshots de respuestas + timeline + diagnóstico mostrado, acción de vinculación post-venta vía `vincular_lead_convertido`, seed de leads en todos los estados. 8 tests nuevos (suite total 1.302, tsc limpio, smoke de listado/detalle/permisos). Pendiente: PR | — |
